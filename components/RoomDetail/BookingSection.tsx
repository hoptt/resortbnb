...
export default function BookingSection({ data }: { data: RoomType }) {
  ...
  const { dayCount, guestCount, checkInDate, checkOutDate } = useRecoilValue(
    calculatedFilterState
  );
  ...
  const handleSubmit = () => {
    router.push(
      ...
      }&guestCount=${guestCount}&totalAmount=${totalAmount}&totalDays=${dayCount}`
    );
  };
